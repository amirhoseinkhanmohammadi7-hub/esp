                ...
                <div class="max-w-[70%] glass-card p-3 {{ $isMe ? 'bg-purple-500/20 border-purple-500/30' : '' }}">
                    @if($message->file_path)
                        @if($message->file_type === 'image')
                            <img src="{{ url('storage/' . $message->file_path) }}" class="rounded-lg max-w-full mb-2">
                        @elseif($message->file_type === 'video')
                            <video controls class="rounded-lg max-w-full mb-2"><source src="{{ url('storage/' . $message->file_path) }}" type="video/mp4"></video>
                        @elseif($message->file_type === 'audio')
                            <div class="voice-message flex items-center gap-2 bg-gradient-to-r from-purple-900/40 to-black/30 backdrop-blur-sm rounded-xl p-3 pr-4 border border-purple-500/20 shadow-lg">
                                <button onclick="toggleVoicePlayback(this)" class="play-btn w-10 h-10 flex items-center justify-center bg-gradient-to-br from-purple-500/40 to-purple-600/50 hover:from-purple-500/60 hover:to-purple-600/70 rounded-full transition-all flex-shrink-0 shadow-md">
                                    <svg class="w-5 h-5 text-white play-icon" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
                                    <svg class="w-5 h-5 text-white pause-icon hidden" fill="currentColor" viewBox="0 0 24 24"><path d="M6 4h4v16H6V4zm8 0h4v16h-4V4z"/></svg>
                                </button>
                                <div class="flex-1 min-w-0 mx-3">
                                    <div class="voice-waveform h-8 flex items-center gap-1">
                                        @for($i = 0; $i < 30; $i++)<div class="w-1 bg-gradient-to-t from-purple-400/70 to-purple-300/90 rounded-full" style="height: {{ rand(20, 100) }}%;"></div>@endfor
                                    </div>
                                </div>
                                <span class="text-xs text-white/60 font-medium ml-3 bg-black/20 px-2 py-1 rounded">{{ $message->created_at->format('H:i') }}</span>
                                <audio class="hidden" src="{{ url('storage/' . $message->file_path) }}"></audio>
                            </div>
                        @else
                            <a href="{{ url('storage/' . $message->file_path) }}" target="_blank" class="text-xs text-cyan-300 hover:underline inline-flex items-center gap-1 mb-2">
...
let lastMessageId={{ $messages->last()?->id ?? 0 }};setInterval(async ()=>{try{const response=await fetch(`/api/chat/{{ $partner->id }}/messages?last_id=${lastMessageId}`),result=await response.json();if(result.success&&result.messages&&result.messages.length>0){result.messages.forEach(msg=>{const isMe=msg.user_id==={{ auth()->id() }};let fileHtml='';if(msg.file_path){const fileUrl='/storage/'+msg.file_path;if(msg.file_type==='image')fileHtml=`<img src="${fileUrl}" class="rounded-lg max-w-full mb-2">`;else if(msg.file_type==='video')fileHtml=`<video controls class="rounded-lg max-w-full mb-2"><source src="${fileUrl}" type="video/mp4"></video>`;else if(msg.file_type==='audio'){const createdAt=new Date(msg.created_at);const timeStr=createdAt.toLocaleTimeString('fa-IR',{hour:'2-digit',minute:'2-digit'});fileHtml=`<div class="voice-message flex items-center gap-2 bg-gradient-to-r from-purple-900/40 to-black/30 backdrop-blur-sm rounded-xl p-3 pr-4 border border-purple-500/20 shadow-lg"><button onclick="toggleVoicePlayback(this)" class="play-btn w-10 h-10 flex items-center justify-center bg-gradient-to-br from-purple-500/40 to-purple-600/50 hover:from-purple-500/60 hover:to-purple-600/70 rounded-full transition-all flex-shrink-0 shadow-md"><svg class="w-5 h-5 text-white play-icon" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg><svg class="w-5 h-5 text-white pause-icon hidden" fill="currentColor" viewBox="0 0 24 24"><path d="M6 4h4v16H6V4zm8 0h4v16h-4V4z"/></svg></button><div class="flex-1 min-w-0 mx-3"><div class="voice-waveform h-8 flex items-center gap-1">${Array(30).fill(0).map(()=>`<div class="w-1 bg-gradient-to-t from-purple-400/70 to-purple-300/90 rounded-full" style="height:${Math.floor(Math.random()*80+20)}%"></div>`).join('')}</div></div><span class="text-xs text-white/60 font-medium ml-3 bg-black/20 px-2 py-1 rounded">${timeStr}</span><audio class="hidden" src="${fileUrl}"></audio></div>`}}const createdAt=new Date(msg.created_at);const msgTimeStr=createdAt.toLocaleTimeString('fa-IR',{hour:'2-digit',minute:'2-digit'});const messageHtml=`<div class="flex ${isMe?'justify-end':'justify-start'}"><div class="max-w-[70%] glass-card p-3 ${isMe?'bg-purple-500/20 border-purple-500/30':''}">${fileHtml}${msg.message?`<p class="text-sm whitespace-pre-wrap">${escapeHtml(msg.message)}</p>`:''}<div class="text-[10px] text-white/40 mt-1 text-left message-time">${msgTimeStr}</div></div></div>`;messagesContainer.insertAdjacentHTML('beforeend',messageHtml);lastMessageId=msg.id});messagesContainer.scrollTop=messagesContainer.scrollHeight}}catch(error){console.error('Error fetching new messages:',error)}},2000);
...
sendVoiceBtn.addEventListener('click',async ()=>{if(!previewAudio.src)return;const blob=await fetch(previewAudio.src).then(r=>r.blob()),formData=new FormData();formData.append('userId','{{ $partner->id }}');formData.append('file',blob,'voice_message.webm');try{const response=await fetch(`/chat/{{ $partner->id }}/send`,{method:'POST',headers:{'X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]').content},body:formData}),result=await response.json();if(result.success){const msg=result.message,fileUrl='/storage/'+msg.file_path,createdAt=new Date(msg.created_at),timeStr=createdAt.toLocaleTimeString('fa-IR',{hour:'2-digit',minute:'2-digit'}),messageHtml=`<div class="flex justify-end"><div class="max-w-[70%] glass-card p-3 bg-purple-500/20 border-purple-500/30"><div class="voice-message flex items-center gap-2 bg-gradient-to-r from-purple-900/40 to-black/30 backdrop-blur-sm rounded-xl p-3 pr-4 border border-purple-500/20 shadow-lg"><button onclick="toggleVoicePlayback(this)" class="play-btn w-10 h-10 flex items-center justify-center bg-gradient-to-br from-purple-500/40 to-purple-600/50 hover:from-purple-500/60 hover:to-purple-600/70 rounded-full transition-all flex-shrink-0 shadow-md"><svg class="w-5 h-5 text-white play-icon" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg><svg class="w-5 h-5 text-white pause-icon hidden" fill="currentColor" viewBox="0 0 24 24"><path d="M6 4h4v16H6V4zm8 0h4v16h-4V4z"/></svg></button><div class="flex-1 min-w-0 mx-3"><div class="voice-waveform h-8 flex items-center gap-1">${Array(30).fill(0).map(()=>`<div class="w-1 bg-gradient-to-t from-purple-400/70 to-purple-300/90 rounded-full" style="height:${Math.floor(Math.random()*80+20)}%"></div>`).join('')}</div></div><span class="text-xs text-white/60 font-medium ml-3 bg-black/20 px-2 py-1 rounded">${timeStr}</span><audio class="hidden" src="${fileUrl}"></audio></div><div class="text-[10px] text-white/40 mt-2 text-left message-time">${timeStr}</div></div></div>`;messagesContainer.insertAdjacentHTML('beforeend',messageHtml);messagesContainer.scrollTop=messagesContainer.scrollHeight;voicePreview.classList.add('hidden');previewAudio.src='';previewAudio.load();isPlayingPreview=false;playPreviewBtn.querySelector('.play-icon').classList.remove('hidden');playPreviewBtn.querySelector('.pause-icon').classList.add('hidden');recordingDuration.textContent='00:00';lastMessageId=msg.id}}catch(error){alert('خطا در ارسال ویس')}});
chatForm.addEventListener('submit',async (e)=>{e.preventDefault();const formData=new FormData(chatForm),userId=formData.get('userId');try{const response=await fetch(`/chat/${userId}/send`,{method:'POST',headers:{'X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]').content},body:formData}),result=await response.json();if(result.success){const msg=result.message;let fileHtml='';if(msg.file_path){const fileUrl='/storage/'+msg.file_path,createdAt=new Date(msg.created_at),timeStr=createdAt.toLocaleTimeString('fa-IR',{hour:'2-digit',minute:'2-digit'});if(msg.file_type==='image')fileHtml=`<img src="${fileUrl}" class="rounded-lg max-w-full mb-2">`;else if(msg.file_type==='video')fileHtml=`<video controls class="rounded-lg max-w-full mb-2"><source src="${fileUrl}" type="video/mp4"></video>`;else if(msg.file_type==='audio')fileHtml=`<div class="voice-message flex items-center gap-2 bg-gradient-to-r from-purple-900/40 to-black/30 backdrop-blur-sm rounded-xl p-3 pr-4 border border-purple-500/20 shadow-lg"><button onclick="toggleVoicePlayback(this)" class="play-btn w-10 h-10 flex items-center justify-center bg-gradient-to-br from-purple-500/40 to-purple-600/50 hover:from-purple-500/60 hover:to-purple-600/70 rounded-full transition-all flex-shrink-0 shadow-md"><svg class="w-5 h-5 text-white play-icon" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg><svg class="w-5 h-5 text-white pause-icon hidden" fill="currentColor" viewBox="0 0 24 24"><path d="M6 4h4v16H6V4zm8 0h4v16h-4V4z"/></svg></button><div class="flex-1 min-w-0 mx-3"><div class="voice-waveform h-8 flex items-center gap-1">${Array(30).fill(0).map(()=>`<div class="w-1 bg-gradient-to-t from-purple-400/70 to-purple-300/90 rounded-full" style="height:${Math.floor(Math.random()*80+20)}%"></div>`).join('')}</div></div><span class="text-xs text-white/60 font-medium ml-3 bg-black/20 px-2 py-1 rounded">${timeStr}</span><audio class="hidden" src="${fileUrl}"></audio></div>`;else fileHtml=`<a href="${fileUrl}" target="_blank" class="text-xs text-cyan-300 hover:underline inline-flex items-center gap-1 mb-2"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg> دانلود فایل</a>`}const messageHtml=`<div class="flex justify-end"><div class="max-w-[70%] glass-card p-3 bg-purple-500/20 border-purple-500/30">${fileHtml}${msg.message?`<p class="text-sm whitespace-pre-wrap">${escapeHtml(msg.message)}</p>`:''}<div class="text-[10px] text-white/40 mt-1 text-left message-time">${timeStr}</div></div></div>`;messagesContainer.insertAdjacentHTML('beforeend',messageHtml);messagesContainer.scrollTop=messagesContainer.scrollHeight;messageInput.value='';fileInput.value='';lastMessageId=msg.id}else{alert('❌ '+result.message)}}catch(error){alert('خطا در ارسال پیام')}});
